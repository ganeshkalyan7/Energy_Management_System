...
import "./LTOStorageSystemControl.css"
...
function LTOStorageSystemControl() {
  const [LTODetails,setLTODetails]=useState([])
  const LTODetails_API=`${nodeAdress}/battery/lto`
...
const ControlModeSelector=(value)=>{
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(LTODetails_API);
...
for(let i=0;i<LTODetails.length;i++){
  ...
  PrechargeContactorStatus=LTODetails[i].prechargeContactorStatus
}
...
const handleEndDateChange = (date) => {
  ...
  return (
    <div>
       <div style={{fontSize:"18px",fontWeight:"600"}}>LTO Battery Overview</div>
       <br/>
...
export default LTOStorageSystemControl

...
function BuildindConsumptionPage2() {
  ...
  const [buildingHighlightsDateFilter, setBuildingHighlightsDateFilter] =
    useState([]);
  ...
  const fetchSystemOverViewData = async () => {
    //setLoading(true);
    try {
      const formattedDate = systemOverviewfilterDate
        ? new Date(
            systemOverviewfilterDate.getTime() -
              systemOverviewfilterDate.getTimezoneOffset() * 60000
          )
            .toISOString()
            .substring(0, 10)
        : "";
      ...
      const response = await axios.post(
        `${analyticsAdress}/filtered/fiveminWise`,
        { date: formattedDate }
      );
      const buildingHighlightsResponse = await axios.post(
        `${bmssAdress}/filtered/buildingConsumptionHighlights`,
        { date: formattedDate }
      );
    ...
    xAxis: {
      categories:
        systemOverviewfilterDate == null
          ? graph.map((time) => time.polledTime)
          : data.map((val) => val.polledTime),
      crosshair: true,
      tickInterval: 12 * 2,
    },
    yAxis: [
    ...
    series: [
      {
        ...
        data:
          systemOverviewfilterDate == null
            ? graph.map((val) => val.wheeledEnergy)
            : data.map((val) => val.wheeledEnergy),
        color: "#F17E50",
        //type: 'column'
        //yAxis: 0,
        marker: {
...

      {
        ...
        data:
          systemOverviewfilterDate == null
            ? graph.map((val) => val.wheeledEnergy2)
            : data.map((val) => val.wheeledEnergy2),
        color: "#D4501B",
        //type: 'column'
        //yAxis: 0,
        marker: {
...

      {
        ...
        data:
          systemOverviewfilterDate == null
            ? graph.map((val) => val.gridEnergy)
            : data.map((val) => val.gridEnergy),
        color: "#7A6464",
        marker: {
          enabled: false, // Disable markers for this series
        },
      },
      {
        ...
        data:
          systemOverviewfilterDate == null
            ? graph.map((val) => val.windEnergy)
            : data.map((val) => val.windEnergy),
        color: "#3B427A",
        marker: {
          enabled: false, // Disable markers for this series
        },
  ...
  const dateValue = systemOverviewfilterDate
    ? new Date(
        systemOverviewfilterDate.getTime() -
          systemOverviewfilterDate.getTimezoneOffset() * 60000
  ...
  return (
    <div style={{ margin: "0px" }}>
      ...
      >
        Grid Consumption(kWh) VS Wheeled In Energy(kWh)
      </p>

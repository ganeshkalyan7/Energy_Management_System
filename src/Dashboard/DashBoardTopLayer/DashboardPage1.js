...
function DashboardPage1({ selectedDate }) {
  ...
  const [energyConsumptionsDateFiltered, setEnergyConsumptionsDateFiltered] =
    useState([]);
    ...
    try {
      const formattedDate = selectedDate
        ? new Date(
            ...
            .substring(0, 10)
        : "";
      const TotalEnergyConsumption = await axios.post(
        energyConsumptionsDateFiltered_API,
        { date: formattedDate }
      );
      const response = await axios.post(maximumPeakDemandDateFiltered_API, {
        date: formattedDate,
      });
      const co2Response = await axios.post(co2ReductionDateFiltered_API, {
        date: formattedDate,
      });
  ...
  if (selectedDate) {
    for (let i = 0; i < energyConsumptionsDateFiltered.length; i++) {
      TotalEnergyConsumedToday = Math.trunc(
        energyConsumptionsDateFiltered[i].Consumption
      );
    }
  } else {
          ...
          <Grid item xs={12}>
            <div className="ReactanleRoot">
              <Grid item xs={4}>
                ...
                >
                  <h4
                    className="TodaysCo2ReductionRoot "
                    style={{
                      fontSize: "18px",
                      fontWeight: "600",
                      marginTop: "-3px",
                  ...
                  >
                    {" "}
                    {TotalEnergyConsumedToday}{" "}
                  </span>
                  <p
                    className="TonsOfCo2Root"
                    style={{
                      color: "black",
                      fontFamily: "Poppins",
                      fontSize: "14px",
                ...
                >
                  ...
                  >
                    ...
                    <FaRegClock
                      ...
                      style={{ marginTop: "-3px" }}
                    />{" "}
                    <span style={{ marginLeft: "10px" }}>{PeakDemandTime}</span>
                  </p>
                </div>
              </Grid>

...
import DashBoardEvchrager from "../DashBoardEvcharger/DashboardEvcharger";
...
function DashBoardSecoundLayer() {
  ...
  const [
    MonthlyEnergyProfile_Renwables_data,
    setMonthlyEnergyProfile_Renwables_data,
  ] = useState([]);
  const [
    MonthlyEnergyProfile_Renwables_data_DateFiltered,
    setMonthlyEnergyProfile_Renwables_data_DateFiltered,
  ] = useState([]);
  ...
  const [
    MonthlyEnergyProfile_SourcesofRenewables_Data,
    setMonthlyEnergyProfile_SourcesofRenewables_Data,
  ] = useState([]);
  const [
    MonthlyEnergyProfile_SourcesofRenewables_DateFiltered_Data,
    setMonthlyEnergyProfile_SourcesofRenewables_DateFiltered_Data,
  ] = useState([]);
  ...
  const fetchYearMonthWiseData = async () => {
    ...
    try {
      const response = await axios.post(
        MonthlyEnergyProfile_Renwables_Filtered_API,
        { date: responseStartYear }
      );
      const SourceOfRenwablesResponse = await axios.post(
        MonthlyEnergyProfile_SourcesofRenewables_DateFiltered_API,
        { date: responseStartYear }
      );
      setMonthlyEnergyProfile_Renwables_data_DateFiltered(response.data);
      setMonthlyEnergyProfile_SourcesofRenewables_DateFiltered_Data(
        SourceOfRenwablesResponse.data
      );
  ...
  const [dashBoardHighlightsDateFiltered, setDashBoardHighlightsDateFiltered] =
    useState([]);
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          MonthlyEnergyProfile_SourcesofRenewables_API
        );
        const dataResponse = res.data;
    ...
    try {
      const formattedDate = selectedDate
        ? new Date(
            ...
            .substring(0, 10)
        : "";
      const response = await axios.post(DashBoardHighlightsDateFiltered_Api, {
        date: formattedDate,
      });
  ...
  if (selectedDate == null) {
    for (let i = 0; i < dashBoardHighlightsdata.length; i++) {
      ...
      Wind =
        dashBoardHighlightsdata[i].wind < 79000
          ? dashBoardHighlightsdata[i].wind
          : 2244;
    ...
    for (let i = 0; i < dashBoardHighlightsDateFiltered.length; i++) {
      ...
      WheeledInsolarPhase2 = Math.trunc(
        dashBoardHighlightsDateFiltered[i].wheeled2
      );
  ...
  values.push(
    Math.round(GridEnegy),
    Math.trunc(RoofTopSolar),
    Math.trunc(WheeledInsolar),
    Diesel,
    WheeledInsolarPhase2,
    Wind
  );
  ...
  if (selectedYear == null) {
    for (let i = 0; i < MonthlyEnergyProfile_Renwables_data.length; i++) {
      ...
      RE_Percentage =
        MonthlyEnergyProfile_Renwables_data[i].RE_Percentage > 100
          ? 100
          : MonthlyEnergyProfile_Renwables_data[i].RE_Percentage;
    }
  } else {
    for (
      let i = 0;
      i < MonthlyEnergyProfile_Renwables_data_DateFiltered.length;
      i++
    ) {
      Total_RE = MonthlyEnergyProfile_Renwables_data_DateFiltered[i].Total_RE;
      Total_fossil =
        MonthlyEnergyProfile_Renwables_data_DateFiltered[i].Total_fossil;
      RE_Percentage =
        MonthlyEnergyProfile_Renwables_data_DateFiltered[i].RE_Percentage > 100
          ? 100
          : MonthlyEnergyProfile_Renwables_data_DateFiltered[i].RE_Percentage;
    }
  }

  console.log(RE_Percentage);
  ...
  if (selectedYear == null) {
    for (
      let i = 0;
      i < MonthlyEnergyProfile_SourcesofRenewables_Data.length;
      i++
    ) {
      WheeledMonth = Math.trunc(
        MonthlyEnergyProfile_SourcesofRenewables_Data[i].wheeledph1_percentage
      );
      WheeledMonthPhase2 = Math.trunc(
        MonthlyEnergyProfile_SourcesofRenewables_Data[i].wheeledph2_percentage
      );
      RoofMonth = Math.trunc(
        MonthlyEnergyProfile_SourcesofRenewables_Data[i].rooftop_percentage
      );
      WindMonth = Math.trunc(
        MonthlyEnergyProfile_SourcesofRenewables_Data[i].wind_percentage
      );
    }
  } else {
    for (
      let i = 0;
      i < MonthlyEnergyProfile_SourcesofRenewables_DateFiltered_Data.length;
      i++
    ) {
      WheeledMonth = Math.round(
        MonthlyEnergyProfile_SourcesofRenewables_DateFiltered_Data[i]
          .wheeledph1_percentage
      );
      WheeledMonthPhase2 = Math.round(
        MonthlyEnergyProfile_SourcesofRenewables_DateFiltered_Data[i]
          .wheeledph2_percentage
      );
      RoofMonth = Math.round(
        MonthlyEnergyProfile_SourcesofRenewables_DateFiltered_Data[i]
          .rooftop_percentage
      );
      WindMonth = Math.round(
        MonthlyEnergyProfile_SourcesofRenewables_DateFiltered_Data[i]
          .wind_percentage
      );
    }
              ...
              <Grid item xs={6}>
                <div className="childcontainerleft" style={{ marginTop: "4%" }}>
                  ...
                  >
                    ...
                    <MdOutlineInfo size="18px" color="black" />
                  </span>
                  <div
                    style={{
                    ...
                    >
                      ...
                      >
                        <div class="bar-container" style={{ display: "flex" }}>
                          ...
                          {WheeledMonthPhase2 === 0 || undefined ? (
                            ""
                          ) : (
                            <div
                              class="bar white"
                              style={{
                                width: `${WheeledMonthPhase2}%`,
                                fontSize: "12px",
                                fontWeight: "500",
                                color: "#2B2B2B",
                                textAlign: "center",
                            ...
                            >
                              Solar With <br /> Trackers
                            </div>
                          )}

                          {WindMonth === 0 || undefined ? (
                            ""
                          ) : (
                            <div
                              class="bar white"
                        ...
                        <div class="bar-container" style={{ display: "flex" }}>
                          ...
                          {WheeledMonthPhase2 === 10 || undefined ? (
                            ""
                          ) : (
                            <div
                              class="bar white"
                    ...
                    <Grid container spacing={1} justifyContent="space-between">
                      <Grid item xs={6}>
                        ...
                        <p style={{ marginLeft: "5%" }}>
                          <TbRectangleFilled color="#7A6464" size="30px" /> Grid
                        </p>
                        <p
                          style={{
                            marginLeft: "5%",
                        ...
                        >
                          <TbRectangleFilled color="#3B427A" size="30px" />
                          Wind
                        </p>
                        <p style={{ marginLeft: "5%", marginTop: "-20px" }}>
                          <TbRectangleFilled color="#9D86A5" size="30px" />{" "}
                          RoofTop Solar
                        </p>
                        ...
                        >
                          <TbRectangleFilled color="#F17E50" size="30px" />
                          Solar Without Trackers
                        </p>
                        ...
                        >
                          <TbRectangleFilled color="#D4501B" size="30px" />
                          Solar With Trackers
                        </p>

                        <p style={{ marginLeft: "5%", marginTop: "-20px" }}>
                          <TbRectangleFilled color="#303030" size="30px" />{" "}
                          Diesel
                        </p>
